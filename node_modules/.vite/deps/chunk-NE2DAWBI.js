import {
  getSignPayload
} from "./chunk-4336AQLA.js";
import {
  fromRpc,
  fromRpc2,
  toRpc,
  toRpc2
} from "./chunk-N4ZY67I3.js";
import {
  HDKey,
  ens_normalize,
  generateMnemonic,
  ...
  wordlist8,
  wordlist9
} from "./chunk-H43UQQNF.js";
import {
  import_index
} from "./chunk-MZYXOCVJ.js";
import {
  createClient,
  prepareAuthorization
} from "./chunk-U7IJITDK.js";
import {
  ...
  return function tonelliSlow(Fp3, n2) {
    ...
    while (!Fp3.eql(b2, Fp3.ONE)) {
      if (Fp3.eql(b2, Fp3.ZERO))
        return Fp3.ZERO;
      let m2 = 1;
      for (let t22 = Fp3.sqr(b2); m2 < r3; m2++) {
        if (Fp3.eql(t22, Fp3.ONE))
          break;
        t22 = Fp3.sqr(t22);
      }
      const ge = Fp3.pow(g2, _1n2 << BigInt(r3 - m2 - 1));
      g2 = Fp3.sqr(ge);
      x2 = Fp3.mul(x2, ge);
      b2 = Fp3.mul(b2, g2);
      r3 = m2;
...
function FpSqrt(P2) {
  ...
  if (P2 % _8n === _5n) {
    ...
    return function sqrt5mod8(Fp3, n2) {
      ...
      const i3 = Fp3.mul(Fp3.mul(nv, _2n2), v3);
...
function weierstrassPoints(opts) {
  ...
  function normPrivateKeyToScalar(key) {
    ...
    if (wrapPrivateKey)
      num2 = mod(num2, N2);
    aInRange("private key", num2, _1n4, N2);
    return num2;
  }
  ...
  class Point2 {
     ...
     */
    static fromHex(hex) {
      const P2 = Point2.fromAffine(fromBytes8(ensureBytes("pointHex", hex)));
      P2.assertValidity();
      return P2;
    }
    ...
    multiplyAndAddUnsafe(Q, a3, b2) {
      const G = Point2.BASE;
      const mul = (P2, a4) => a4 === _0n4 || a4 === _1n4 || !P2.equals(G) ? P2.multiplyUnsafe(a4) : P2.multiply(a4);
...
function weierstrass(curveDef) {
  ...
  const slcNum = (b2, from32, to3) => bytesToNumberBE(b2.slice(from32, to3));
  class Signature {
  ...
  function prepSig(msgHash, privateKey, opts = defaultSigOpts) {
    ...
    const { hash: hash9, randomBytes: randomBytes2 } = CURVE;
    ...
    if (prehash)
      msgHash = ensureBytes("prehashed msgHash", hash9(msgHash));
    ...
    function k2sig(kBytes) {
      ...
      if (r3 === _0n4)
        return;
      const s3 = modN2(ik * modN2(m2 + r3 * d3));
      if (s3 === _0n4)
        return;
  ...
  function verify6(signature, msgHash, publicKey, opts = defaultVerOpts) {
    ...
    const isObj = !isHex2 && !format9 && typeof sg === "object" && sg !== null && typeof sg.r === "bigint" && typeof sg.s === "bigint";
    if (!isHex2 && !isObj)
    ...
    const R = (_a = Point2.BASE.multiplyAndAddUnsafe(P2, u1, u22)) == null ? void 0 : _a.toAffine();
    if (!R)
      return false;
    const v3 = modN2(R.x);
    return v3 === r3;
...
function SWUFpSqrtRatio(Fp3, Z) {
  const q2 = Fp3.ORDER;
  let l3 = _0n4;
  for (let o3 = q2 - _1n4; o3 % _2n3 === _0n4; o3 /= _2n3)
    l3 += _1n4;
  const c1 = l3;
  ...
  const c7 = Fp3.pow(Z, (c22 + _1n4) / _2n3);
  let sqrtRatio = (u3, v3) => {
    let tv1 = c6;
...
function getHash(hash9) {
  return {
    hash: hash9,
    hmac: (key, ...msgs) => hmac(hash9, key, concatBytes2(...msgs)),
    randomBytes
  };
}
function createCurve(curveDef, defHash) {
  const create4 = (hash9) => weierstrass({ ...curveDef, ...getHash(hash9) });
...
function hash_to_field(msg, count, options) {
  ...
  const { p: p3, k: k3, m: m2, hash: hash9, expand, DST: _DST } = options;
  abytes(msg);
  anum(count);
  ...
  if (expand === "xmd") {
    ...
    prb = expand_message_xof(msg, DST, len_in_bytes, k3, hash9);
...
function lift_x(x2) {
  aInRange("x", x2, _1n5, secp256k1P);
  const xx = modP(x2 * x2);
  const c3 = modP(xx * x2 + BigInt(7));
  let y3 = sqrtMod(c3);
  if (y3 % _2n4 !== _0n5)
    y3 = modP(-y3);
  const p3 = new Point(x2, y3, _1n5);
  p3.assertValidity();
  return p3;
}
...
function schnorrVerify(signature, message, publicKey) {
  ...
  try {
    ...
    if (!inRange(s3, _1n5, secp256k1N))
      return false;
    const e3 = challenge(numTo32b(r3), pointToBytes(P2), m2);
    const R = GmulAdd(P2, s3, modN(-e3));
    if (!R || !R.hasEvenY() || R.toAffine().x !== r3)
      return false;
    return true;
  } catch (error) {
    return false;
...
var swap8IfBE = isLE2 ? (n2) => n2 : (n2) => byteSwap(n2);
...
var swap32IfBE = isLE2 ? (u3) => u3 : byteSwap32;
var hasHexBuiltin = (() => (
  // @ts-ignore
...
function Chi2(a3, b2, c3) {
  return a3 & b2 ^ ~a3 & c3;
}
function Maj2(a3, b2, c3) {
...
var SHA3_ROTL = [];
var _SHA3_IOTA = [];
for (let round = 0, R = _1n6, x2 = 1, y3 = 0; round < 24; round++) {
...
var SHA2562 = class extends HashMD2 {
  ...
  process(view, offset) {
    ...
    for (let i3 = 0; i3 < 64; i3++) {
      ...
      const T22 = sigma0 + Maj2(A2, B3, C3) | 0;
      H = G;
      G = F2;
      F2 = E;
      E = D2 + T1 | 0;
...
var sha2562 = createHasher2(() => new SHA2562());
...
function validate7(value) {
  return validate2(value) && size4(value) === 256;
}
...
var bls12_381 = bls({
  ...
  G2: {
    ...
    fromBytes: (bytes) => {
      ...
      const L2 = Fp.BYTES;
      const slc = (b2, from32, to3) => bytesToNumberBE(b2.slice(from32, to3));
...
@noble/curves/esm/bls12-381.js:
  (*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=chunk-NE2DAWBI.js.map
